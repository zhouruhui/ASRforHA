...
volcengine_asr:
  ...
  # --- VAD 相关配置 (推荐优化值) ---
  # end_window_size: 2000                             # VAD 检测非语音部分的窗口大小(毫秒)，默认2000
...
**VAD相关配置（推荐优化值）**：
*   `end_window_size` (可选): VAD 检测非语音部分的窗口大小，单位为毫秒。当检测到指定时长的无声音频时，会自动结束识别过程。默认为 `2000`毫秒(2秒)。
    - 如果语音经常被过早截断，可以进一步增大此值到2500-3000；
    - 如果语音响应过慢，可以减小此值到1000-1500。
...
*   **语音被截断，只识别出部分内容**：
    ...
    *   增大 `end_window_size` 参数的值：建议设置为 `2000` 或更高，让系统更耐心地等待您说完整句话。
    *   如果您的语音中包含较长的自然停顿，可以尝试设置更大的值，如 `3000` 或更高。
